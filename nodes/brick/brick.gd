...
var bricks_around: Dictionary[String, Brick] = {
	"tl": null,
	"t": null,
	"tr": null,
	"l": null,
...
func link_mines() -> void:
	bricks_around["tl"] = minefield.get_brick(x - 1, y + 1)
	bricks_around["t"] = minefield.get_brick(x, y + 1)
	bricks_around["tr"] = minefield.get_brick(x + 1, y + 1)
	bricks_around["l"] = minefield.get_brick(x - 1, y)
	bricks_around["r"] = minefield.get_brick(x + 1, y)
	bricks_around["bl"] = minefield.get_brick(x - 1, y - 1)
	bricks_around["b"] = minefield.get_brick(x, y - 1)
	bricks_around["br"] = minefield.get_brick(x + 1, y - 1)


func mines_around_recount(finalized: bool = false) -> int:
	var count := 0
	for brick in bricks_around.values():
		if (brick != null and brick.has_mine):
			count += 1
...
func destroy(delayed: float = 0.0) -> void:
	...
	if (mines_around_count == 0 and not has_mine):
		for brick in bricks_around.values():
			if (brick != null and not brick.is_destroyed):
				brick.destroy(0.15)
